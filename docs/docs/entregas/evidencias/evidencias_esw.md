---
sidebar_label: "Evidências de Engenharia de Software"
sidebar_position: 1
---
import useBaseUrl from '@docusaurus/useBaseUrl';

# Evidências de Engenharia de Software (ESW) 

## **Uso do GitHub Projects**
Durante o curso do projeto, usamos o GitHub Projects para atribuição de tarefas e delegação de responsabilidades.

### Burnup do GitHub Projects até dia 18/11
A figura abaixo mostra o gráfico disponibilizado pela ferramenta nativa Insights do GitHub Projects, que permite a visualização do progresso do projeto.
<a id="burnup_rxhospitalar_projects"></a>
*Figura 1: Burnup do GitHub Projects até o dia 18/11*
<img src={useBaseUrl('/img/burnup_rxhospitalar_projects.png')} alt="Burnup do GitHub Projects" />
*Fonte: De autoria própria.*

### Organização das tarefas dentro do GitHub Projects
A figura abaixo mostra um exemplo da organização das tarefas da disciplina.
<a id="github_projects"></a>
*Figura 2: Tasks no GitHub Projects*
<img src={useBaseUrl('/img/github_projects_18_11_25.png')} alt="Tasks no GitHub Projects" />
*Fonte: De autoria própria.*

## **Uso do Teams**
O Teams foi a ferramenta usada para dailies síncronas, porque permite gravá-las.

### Reuniões via Teams
A figura abaixo apresenta um exemplo de como fazíamos as nossas reuniões mais longas.
<a id="dailies_teams"></a>
*Figura 3: Reuniões no Teams*
<img src={useBaseUrl('/img/dailies_sincronas.png')} alt="Dailies síncronas no Teams" />
*Fonte: De autoria própria.*

## **Uso do Discord**
O Discord foi uma ferramenta usada para comunicação e para se fazer as dailies assíncronas.

### Dailies assíncronas via Discord
A figura abaixo mostra um exemplo da organização das atualizações na daily por via assíncrona. Na maior parte das vezes, elas ocorreram de maneira síncrona.
<a id="dailies_discord"></a>
*Figura 4: Dailies assíncronas no Discord*
<img src={useBaseUrl('/img/dailies_assincronas.png')} alt="Tasks no GitHub Projects" />
*Fonte: De autoria própria.*

### Pair Programming
A figura abaixo apresenta um exemplo de colaboração na hora de fazer o código.
<a id="dailies_discord"></a>
*Figura 5: Programação colaborativa*
<img src={useBaseUrl('/img/pair_programming.png')} alt="Pair Programming no Discord" />
*Fonte: De autoria própria.*

## **Uso do Miro**
Usamos o Miro para confecção das atividades referentes aos estudos de caso e planejamento do projeto.

### Quadro destinado às Sprints
A figura abaixo mostra um exemplo de retrospectiva de uma Sprint
<a id="retrospective_miro"></a>
*Figura 5: Retrospectiva de uma Sprint no Miro*
<img src={useBaseUrl('/img/rrp1.png')} alt="Retrospectiva no Miro" />
*Fonte: De autoria própria.*

## **Protótipos do RAD e validação com o cliente**

### Exemplo de protótipo das telas e validação com o cliente
A figura abaixo mostra como foi o questionário de validação feito para o cliente sobre uma das telas.
<a id="tela_validacao"></a>
*Figura 6: Validação sobre as telas*
<img src={useBaseUrl('/img/tela_prototipo.png')} alt="Validação sobre as telas" />
*Fonte: De autoria própria.*

### Itens de validação para as telas do protótipo
<a id="itens_validacao"></a>
*Figura 7: Itens para a validação*
<img src={useBaseUrl('/img/itens_verificacao.png')} alt="Itens da validação" />
*Fonte: De autoria própria.*

### Resposta do cliente
<a id="resposta_cliente"></a>
*Figura 8: Resposta do cliente*
<img src={useBaseUrl('/img/resposta_feedback.png')} alt="Resposta do Cliente" />
*Fonte: De autoria própria.*

## **Teste de usuário e validação com o cliente**

### Formulário para o teste
<a id="resposta_cliente"></a>
*Figura 9: Formulário para o teste de usuário*
<img src={useBaseUrl('/img/teste_usuario_1.png')} alt="Resposta do Cliente" />
*Fonte: De autoria própria.*

### Exemplo de parte do formulário
<a id="resposta_cliente"></a>
*Figura 10: Exemplo de parte do formulário para o teste de usuário*
<img src={useBaseUrl('/img/teste_usuario_2.png')} alt="Resposta do Cliente" />
*Fonte: De autoria própria.*

### Parte da resposta do cliente
<a id="resposta_cliente"></a>
*Figura 11: Parte da resposta do cliente*
<img src={useBaseUrl('/img/teste_usuario.png')} alt="Resposta do Cliente" />
*Fonte: De autoria própria.*

