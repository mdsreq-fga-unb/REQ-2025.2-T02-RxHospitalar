...
def _to_number(series: pd.Series) -> pd.Series:
    """Função auxiliar para converter valores monetários/numéricos string em float."""
    s = series.astype(str).str.strip()
    s = s.str.replace(".", "", regex=False).str.replace(",", ".", regex=False)
    return pd.to_numeric(s, errors="coerce").fillna(0.0)

# -------------------------------------------------------------------------
# FUNÇÃO DA BRANCH: integra/sazonalidade-cliente-vendedor
# Objetivo: Cálculo genérico a partir de um DataFrame já carregado
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# FUNÇÃO DA BRANCH: plot_cliente_vendedor
# Objetivo: Lógica específica completa (incluindo carga de arquivo e top clientes)
# -------------------------------------------------------------------------
